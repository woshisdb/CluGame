...
/// 倾向于更正面的给玩家
/// </summary>
public class FriendlyDominanceThinkMood:ThinkBaseModule
{
    /// <summary>
    /// 到达多少个 -3 才触发极端负面态度（默认 1）
    /// </summary>
    public int extremeTriggerCount = 1;
    public FriendlyDominanceThinkMood(NpcCardModel npc) : base(npc)
    {
    }
    public override int GetThinkType(Dictionary<NpcThinkType, List<int>> things)
    {
        // 没有该类型事件 → 默认中立态度
        if (!things.ContainsKey(NpcThinkType.Attitude))
            return 0;

        List<int> events = things[NpcThinkType.Attitude];
